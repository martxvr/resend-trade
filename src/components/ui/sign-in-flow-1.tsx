...
export const CanvasRevealEffect = ({
  ...
  colors = [[255, 255, 255]], // Changed to White
  containerClassName,
  dotSize,
  showGradient = true,
  reverse = false,
...
import { toast } from "sonner"; // Using sonner as it was used in Auth.tsx

export const SignInPage = ({ className }: SignInPageProps) => {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState(""); // Add password state
  const [isLogin, setIsLogin] = useState(true); // Toggle for Login/Signup like original
  const [loading, setLoading] = useState(false);

  const { signIn, signUp } = useAuth(); // Use signIn/signUp instead of OTP
      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        <div className="flex flex-1 flex-col lg:flex-row ">
          <div className="flex-1 flex flex-col justify-center items-center">
            <div className="w-full mt-[100px] max-w-sm"> {/* Adjust top margin slightly */}
              <motion.div
              ...
              >
                <div className="space-y-1">
                  <h1 className="text-[2.5rem] font-bold leading-[1.1] tracking-tight text-white">
                    {isLogin ? "Welcome Trader" : "Create Account"}
                  </h1>
                  <p className="text-[1.8rem] text-white/70 font-light">
                    {isLogin ? "Sign in to access your trading rooms" : "Join the trading room experience"}
                  </p>
                </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3">
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-transparent backdrop-blur-[1px] text-white border border-white/60 rounded-full py-3 px-4 focus:outline-none focus:border-white text-center placeholder:text-white/50 transition-colors"
                        required
                        disabled={loading}
                      />
                    </div>
                ...
                <p className="text-xs text-white/40 pt-6">
                  By continuing, you agree to our <Link to="#" className="underline text-white/40 hover:text-white/60 transition-colors">Terms of Service</Link> and <Link to="#" className="underline text-white/40 hover:text-white/60 transition-colors">Privacy Policy</Link>.
                </p>
              </motion.div>
            </div>
          </div>
